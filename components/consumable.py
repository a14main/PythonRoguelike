    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if amount_recovered > 0:
            self.engine.message_log.add_message(
                text=f"You consume the {self.parent.name} and recover {amount_recovered} HP!",
...
class ConfusionConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        self.engine.message_log.add_message(
            text=f"The eyes of the {target.name} look vacant as it starts to stumble around!",
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target:
            self.engine.message_log.add_message(
                text=f"A lightning bolt strikes the {target.name} with a loud thunder, for {self.damage} damage!"
            )
...
class FireballDamageConsumable(Consumable):
    ...
    def get_action(self, consumer: actions.Actor) -> AreaRangedAttackHandler:
        self.engine.message_log.add_message(
            text="Select a target location.",
            fg=color.needs_target
        )
