...
CURSOR_KEYS = {
    ...
    tcod.event.KeySym.LEFT: -10,
    tcod.event.KeySym.RIGHT: 10,
}
...
If an action is returned it will be attempted ad it it's valid then
...
class GameOverEventHandler(EventHandler):
    ...
    def on_quit(self, event: tcod.event.Quit) -> None:
...
class HistoryViewer(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[MainGameEventHandler]:
        ...
        elif event.sym == tcod.event.KeySym.N0:
            self.cursor = 0 # Move directly to the top message when zero is pressed.
        elif event.sym == tcod.event.KeySym.N1:
            self.cursor = self.log_length - 1 # Move directly to the end when 1 is pressed.
...
class AskUserEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        """By defauly any key exits this input handler"""
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: Console) -> None:
        ...
        and the letter to select each one.
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def __init__(self, engine: actions.Engine):
        """Sets the cursor ro the plater when this handler is constructed"""
        super().__init__(engine)
        player = self.engine.player
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key in MOVE_KEYS:
            ...
            return self.on_index_selelcted(*self.engine.mouse_location)
        return super().ev_keydown(event)
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[ActionOrHandler]:
        ...
        if self.engine.game_map.in_bounds(*event.tile):
            if event.button == 1:
                return self.on_index_selelcted(*event.tile)
        return super().ev_mousebuttondown(event)
    
    def on_index_selelcted(self, x: int, y: int) -> Optional[ActionOrHandler]:
...
class LookHAndler(SelectIndexHandler):
    ...
    def on_index_selelcted(self, x: int, y: int) -> MainGameEventHandler:
...
class SingleRangedAttackHandler(SelectIndexHandler):
    ...
    def on_index_selelcted(self, x: int, y: int) -> Action | None:
...
class AreaRangedAttackHandler(SelectIndexHandler):
    ...
    def on_render(self, console: Console) -> None:
        ...
        # Draw a rectangle around the targeted area, so the player can see the affeced tiles.
        console.draw_frame(
            x=x - self.radius - 1,
            y=y - self.radius - 1,
            width=self.radius ** 2,
    ...
    def on_index_selelcted(self, x: int, y: int) -> Optional[Action]:
        return self.callback((x, y))